...
- **Usernames are ASCII-only in a German-language product**: done - usernames now accept Latin-script letters, so `müller` registers. The blocker recorded here was real and is what the change had to answer: `LOWER()` folds only ASCII when the database's lc_ctype is C, so the database could not be the thing deciding that `Müller` and `müller` are one account. `users.username_folded` (migration `0005`) stores the fold that `foldUsername` in `shared/schema.ts` computes, and every lookup and uniqueness check reads it, so the answer no longer varies by deployment. Cyrillic and other scripts are still refused, deliberately: `а` (U+0430) renders as `a`, and nothing here detects confusables.
