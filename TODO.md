...
- [ ] **Filaments duplicate catalog names as free text**: A filament stores its material as free text while the catalog stores it as a row, so the two are matched by name. Both places that do this now compare case-insensitively, but the underlying duplication remains - a renamed catalog entry still silently stops matching. Filaments should reference the catalog instead.
