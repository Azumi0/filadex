...
- [x] **Filaments duplicate catalog names as free text**: closed by `docs/adr/0003-per-user-material-catalog.md`. A declared material that resolves to no Catalog Material is now registered into the declaring user's Personal Catalog (`materials` gained a nullable `user_id`), so every declared material resolves to a row and can be shared per-material and flagged for drying. Phase 1, the backend, is `docs/plans/per-user-material-catalog-backend.md`; phase 2, making the row visible in the UI, is `docs/plans/per-user-material-catalog-ui.md`.
